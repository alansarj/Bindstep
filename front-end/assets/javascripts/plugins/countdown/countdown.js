	...
	$.fn.countdown = function (options) {
		...
		return this.each(function () {
			...
			function getCurrentCountDown() {
				...
				return {
					days: formatNumber(Math.max(0, days), true),
					hours: formatNumber(Math.max(0, hours), false),
					minutes: formatNumber(Math.max(0, minutes), false),
					seconds: formatNumber(Math.max(0, seconds), false)
				};
			}
